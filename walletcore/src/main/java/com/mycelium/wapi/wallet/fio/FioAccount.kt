...
class FioAccount(private val accountContext: FioAccountContext,
                 ...
                 backing: FioAccountBacking,
                 private val accountListener: AccountListener?,
                 blockchainService: FioBlockchainService,
                 address: FioAddress? = null) : AbstractFioAccount(accountContext.currency, credentials,
        backing, blockchainService, FioAccount::class.simpleName, address) {
    ...
    override fun createTx(toAddress: Address, value: Value, gasPrice: Fee, data: TransactionData?): Transaction {
        ...
        val FioTxData = data as? FioTransactionData
        val nonce = FioTxData?.nonce ?: getNewNonce()
        val gasLimit = FioTxData?.gasLimit ?: BigInteger.valueOf(typicalEstimatedTransactionSize.toLong())
        val inputData = FioTxData?.inputData ?: ""
        val fee = FioTxData?.suggestedGasPrice ?: gasPrice.feePerKb.value
        ...
        if (value > calculateMaxSpendableAmount(gasPriceValue, null)) {
            throw InsufficientFundsException(Throwable("Insufficient funds to send " + Convert.fromWei(value.value.toBigDecimal(), Convert.Unit.FioER) +
                    " Fioer with gas price " + Convert.fromWei(gasPriceValue.valueAsBigDecimal, Convert.Unit.GWEI) + " gwei"))
        }
        return FioTransaction(coinType, toAddress.toString(), value, fee, nonce, gasLimit, inputData)
    }

    override fun signTx(request: Transaction, keyCipher: KeyCipher?) {
        val rawTransaction = (request as FioTransaction).run {
    ...
    override fun broadcastTx(tx: Transaction): BroadcastResult {
        try {
            val result = blockchainService.sendTransaction((tx as FioTransaction).signedHex!!)
            if (!result.success) {
    ...
    private fun syncTransactions() {
        try {
            ...
            logger.log(Level.SEVERE, "Error retrieving Fio/ERC-20 transaction history: ${e.javaClass} ${e.localizedMessage}")
        }
    }
    ...
    override fun calculateMaxSpendableAmount(gasPrice: Value, ign: FioAddress?): Value {
