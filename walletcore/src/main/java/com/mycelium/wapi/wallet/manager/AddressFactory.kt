...
class AddressFactory(val coinType: CryptoCurrency, val networkParameters: CommonNetworkParameters) {

    fun getAddress(publicKey: PublicKey, addressType: AddressType): BtcvAddress? =
            publicKey.toAddress(coinType, networkParameters, addressType)

    fun bytesToAddress(bytes: ByteArray, path: HdKeyPath): BtcvAddress? {
        return try {
