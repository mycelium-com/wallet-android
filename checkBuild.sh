...
releaseFile=mbw-prodnet-release
revision=v2.12.0.18
...
printf "Checking $releaseFile against revision $revision.\nYou can monitor progress in ${workdir}progress.log\n"
printf "Checking $releaseFile against revision $revision ...\n" > ${workdir}progress.log

cp -r . $workdir
cd $workdir

java -jar ~/Downloads/apktool.jar d --output original $releaseFile.apk
...
java -jar ~/Downloads/apktool.jar d candidate.apk
