...
public class AndroidApiCache extends ApiCache {
   ...
   protected TransactionInventory getTransactionInventory(Collection<Address> addresses) {
      // Using a sorted set ensures that the elements are sorted and unique.
      // This removes duplicates which happen when a transaction is referenced
      // from more than one of our addresses
      SortedSet<ApiCache.TransactionInventory.Item> items = new TreeSet<ApiCache.TransactionInventory.Item>();
      int maxHeight = -1;
      ...
      LinkedList<Item> list = new LinkedList<ApiCache.TransactionInventory.Item>(items);
      return new TransactionInventory(list, maxHeight);
   }
