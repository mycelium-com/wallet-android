...
public class Bip39Test {
   ...
   private static final String TEST_NO_PASSWORD_WORDS = "degree rain vendor coffee push math onion inside pyramid blush stick treat";
   private static final String TEST_NO_PASSWORD_SEED = "a359cf47a6ddcc581f28133062dbc4cfacfbd79703e167b2a61438bf9f89efe5f2f12f8d39aea44709a5913965be93a5f805c6c614dcbfe620bceb161f0018c4";

   private static final String TEST_INVALID_CHECKSUM_WORDS_1 = "rain degree vendor coffee push math onion inside pyramid blush stick treat";
   private static final String TEST_INVALID_CHECKSUM_WORDS_2 = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon treat";
   private static final String TEST_INVALID_WORD_COUNT = "degree rain vendor coffee push math onion inside pyramid blush stick";
   private static final String TEST_INVALID_WORDS = "mycelium rain vendor coffee push math onion inside pyramid blush stick treat";
   ...
   public void testEnglishVectorsRawToWords() {
      // Test that the word list we generate from the raw entropy matches with the test vectors
      for (int i = 0; i < TEST_VECTORS.length / 3; i++) {
         String raw = TEST_VECTORS[i * 3];
         String[] wordList = TEST_VECTORS[i * 3 + 1].split(" ");
         String[] myWordList = Bip39.rawEntropyToWords(HexUtils.toBytes(raw));
         assertArrayEquals(wordList, myWordList);
      }
   }

   @Test
   public void testEnglishVectorsWordsToSeeds() {
      // Test that the seed we generate from the word list matches the test vectors
      for (int i = 0; i < TEST_VECTORS.length / 3; i++) {
         String[] wordList = TEST_VECTORS[i * 3 + 1].split(" ");
   ...
   public void testEnglishVectors_WordListChecksum() {
      for (int i = 0; i < TEST_VECTORS.length / 3; i++) {
         // Test that the check sum of the word lists is valid
         String[] wordList = TEST_VECTORS[i * 3 + 1].split(" ");
         assertTrue(Bip39.isValidWordList(wordList));
      }
   }
   ...
   public void testEnglishVectors_WordListInvalid() {
      // Test that the check sum of the word lists is valid
      String[] wordList;
      wordList = TEST_INVALID_CHECKSUM_WORDS_1.split(" ");
      assertFalse(Bip39.isValidWordList(wordList));
      wordList = TEST_INVALID_CHECKSUM_WORDS_2.split(" ");
      assertFalse(Bip39.isValidWordList(wordList));
      wordList = TEST_INVALID_WORD_COUNT.split(" ");
      assertFalse(Bip39.isValidWordList(wordList));
      wordList = TEST_INVALID_WORDS.split(" ");
      assertFalse(Bip39.isValidWordList(wordList));
   }
