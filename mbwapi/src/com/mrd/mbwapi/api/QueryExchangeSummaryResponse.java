...
public class QueryExchangeSummaryResponse extends ApiObject {
   ...
   public QueryExchangeSummaryResponse(ExchangeSummary[] exchangeSummaries) {
      this.exchangeSummaries = exchangeSummaries;
   }

   protected QueryExchangeSummaryResponse(ByteReader reader) throws InsufficientBytesException, ApiException {
      int num = reader.getIntLE();
      exchangeSummaries = new ExchangeSummary[num];
      for (int i = 0; i < num; i++) {
         exchangeSummaries[i] = ApiObject.deserialize(ExchangeSummary.class, reader);
      }
   ...
   protected ByteWriter toByteWriter(ByteWriter writer) {
      writer.putIntLE(exchangeSummaries.length);
      for (ExchangeSummary summary : exchangeSummaries) {
